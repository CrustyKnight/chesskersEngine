...
class Main:
    ...
    ):  # Start and end are tuples (row, col) of board squares.
    ...
    def __move__(self, surface):
        ...
        (
            self.__jump__(surface, start, taken, end)
            if boardUCN.__contains__("t")
            else self.__step__(surface, start, end)
        )
